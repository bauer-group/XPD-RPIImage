...
def render_hardware(cfg: dict[str, Any]) -> None:
    ...
    if wd.get("enabled"):
        ...
        # release silent and what lets the module queue an activation intent
        # from observed change instead of from having been run. A heredoc
        # rewrites the file every time and would make both impossible.
        write(
            gen / "system.conf.d" / "10-bgrpiimage-watchdog.conf",
            "# Auto-generated by bgrpiimage-hardware - do not edit by hand.\n"
            "[Manager]\n"
            f"RuntimeWatchdogSec={wd.get('runtime_sec', WATCHDOG_RUNTIME_SEC_DEFAULT)}\n"
            f"RebootWatchdogSec={wd.get('reboot_sec', WATCHDOG_REBOOT_SEC_DEFAULT)}\n",
        )

    audio = cfg.get("audio") or {}
    if audio.get("default_output"):
        env_chunks.append(shell_var("BGRPIIMAGE_AUDIO_DEFAULT_OUTPUT", audio["default_output"]))
        # Same reasoning as the watchdog drop-in: a generated file, so the
        # apply script installs it by content instead of re-writing it.
        pcm = _ALSA_PCM_BY_OUTPUT.get(audio["default_output"])
        if pcm:
            write(
                gen / "alsa" / "99-bgrpiimage-default.conf",
                "# Auto-generated by bgrpiimage-hardware.\n"
                f'pcm.!default {{ type plug; slave.pcm "{pcm}"; }}\n'
...
def render_podman(cfg: dict[str, Any]) -> None:
    ...
    write(
        ...
        "set -uo pipefail\n"
        "\n"
        "# The archive is a dump of portainer_data: Portainer's admin password\n"
        "# hash, its JWT signing secret and any stored registry/environment\n"
        "# credentials. umask + an explicit 700 dir keep it unreadable by any\n"
        "# other local account under systemd's default UMask=0022.\n"
        "umask 077\n"
        "\n"
        f"KEEP={keep}\n"
...
def render_portainer(cfg: dict[str, Any]) -> None:
    ...
    # Fully qualified: podman resolves unqualified names through
    # unqualified-search-registries, which this image does not ship, and
    # AutoUpdate=registry requires a fully qualified reference regardless.
    image = p.get("image") or (
        "docker.io/portainer/portainer-ce:2.45.0"
        if edition == "ce"
        else "docker.io/portainer/portainer-ee:2.45.0"
    )
    ...
    if (cfg.get("podman") or {}).get("enabled"):
        ...
        # [Install] is what actually wires the generated unit into
        # multi-user.target - Quadlet applies it itself at boot, "in the same
        # way systemctl enable does when run later" (podman-systemd.unit(5)).
        # Omitting the whole section is therefore how `auto_start: false` is
        # honoured under Podman: Quadlet still generates portainer.service,
        # but nothing pulls it in at boot, so it stays inert until started by
        # hand. Shipping [Install] unconditionally (the pre-fix behaviour)
        # started Portainer regardless of this setting.
        install_section = (
            "\n[Install]\nWantedBy=multi-user.target\n"
...
def _semantic_validate(cfg: dict[str, Any]) -> None:
    ...
    # Podman resolves an unqualified image name through
    # unqualified-search-registries, which this image does not ship, so a
    # short name like "portainer/portainer-ce:lts" fails to pull at first
    # boot with the failure buried inside portainer-image.service - a
    # dependency, so Restart=always on the container unit never retries it.
    # Catches the likely operator error of hand-setting a short image name;
    # the code's own fallback (render_portainer()) is always fully qualified.
    if podman_on and portainer.get("enabled") and portainer.get("image"):
        _img = str(portainer["image"])
        _host = _img.split("/", 1)[0]
        if not ("." in _host or ":" in _host or _host == "localhost"):
            raise ValueError(
                f"portainer.image={_img!r} has no registry host - podman "
                "cannot resolve short image names without "
                "unqualified-search-registries, which this image does not "
                "ship. Use a fully qualified reference, e.g. "
