...
Usage:
    python scripts/generate.py config/variants/can-app.json
"""
...
def render_base(cfg: dict[str, Any]) -> None:
    gen = clean_generated("bgos-base")
    write(gen / "hostname", cfg["hostname"] + "\n")
    locale = cfg.get("locale", {})
    lines = [
        shell_var("BGOS_TIMEZONE", locale.get("timezone", "UTC")),
        shell_var("BGOS_LOCALE", locale.get("locale", "en_US.UTF-8")),
        shell_var("BGOS_KEYBOARD", locale.get("keyboard", "us")),
    ]
...
def render_users(cfg: dict[str, Any]) -> None:
    gen = clean_generated("bgos-users")
    users = cfg.get("users", [])
    ...
    for user in users:
        ...
        if user.get("sudo_nopasswd"):
            ...
            script.append(
                f"echo {shlex.quote(sudoers_line)} > /etc/sudoers.d/010-bgos-{name}"
            )
            script.append(f"chmod 440 /etc/sudoers.d/010-bgos-{name}")
        ...
        if keys:
            ...
            script.append(f"cat > {authfile} <<'__BGOS_EOF__'")
            script.extend(keys)
            script.append("__BGOS_EOF__")
    ...
    if su_users:
        ...
        script.append("install -m 644 /tmp/_bgos_su_pam /etc/pam.d/su")
    ...
    script.append("cat > /etc/ssh/sshd_config.d/10-bgos.conf <<'__BGOS_EOF__'")
    script.extend(sshd)
    script.append("__BGOS_EOF__")
    script.append("chmod 644 /etc/ssh/sshd_config.d/10-bgos.conf")

    write(gen / "create-users.sh", "\n".join(script) + "\n", executable=True)

    # /etc/pam.d/su drop-in enabling pam_wheel trust.
    pam_su = (
        "# /etc/pam.d/su - generated by bgos\n"
...
def render_network(cfg: dict[str, Any]) -> None:
    gen = clean_generated("bgos-network")
    net = cfg.get("network", {})
...
def render_boot(cfg: dict[str, Any]) -> None:
    ...
    for extra in boot.get("extra_lines", []):
        lines.append(extra)
    lines.append("")
    write(gen / "config-bgos.txt", "\n".join(lines))


def render_can(cfg: dict[str, Any]) -> None:
    gen = clean_generated("bgos-can")
    can = cfg.get("can", {})
...
def render_docker(cfg: dict[str, Any]) -> None:
    gen = clean_generated("bgos-docker")
...
def render_portainer(cfg: dict[str, Any]) -> None:
    gen = clean_generated("bgos-portainer")
    ...
    write(
        ...
        "".join(
            [
                shell_var("BGOS_PORTAINER_AUTOSTART", bool(p.get("auto_start", True))),
                shell_var("BGOS_PORTAINER_IMAGE", image),
            ]
        ),
    )
...
def render_unattended(cfg: dict[str, Any]) -> None:
    gen = clean_generated("bgos-unattended-upgrades")
    ...
    if reboot.get("enabled"):
        ...
        check_script = [
            ...
            '  echo "bgos-reboot: outside window ($now not in $WINDOW_START..$WINDOW_END)"',
            '  exit 0',
            'fi',
            'if [[ $IF_REQUIRED_ONLY -eq 1 ]]; then',
            '  if [[ ! -f /var/run/reboot-required ]]; then',
            '    echo "bgos-reboot: no reboot required"',
            '    exit 0',
            '  fi',
            'fi',
            'logger -t bgos-reboot "rebooting inside window ${WINDOW_START}-${WINDOW_END}"',
            '/sbin/shutdown -r +1 "bgos: scheduled reboot after unattended-upgrade"',
            '',
        ]
        write(gen / "bgos-reboot-window.sh", "\n".join(check_script), executable=True)
        ...
        svc = (
            ...
            "[Service]\n"
            "Type=oneshot\n"
            "ExecStart=/usr/local/sbin/bgos-reboot-window.sh\n"
        )
        write(gen / "bgos-reboot-window.service", svc)
        ...
        tmr = (
            ...
            "Persistent=true\n"
            "Unit=bgos-reboot-window.service\n"
            "\n"
            "[Install]\n"
            "WantedBy=timers.target\n"
        )
        write(gen / "bgos-reboot-window.timer", tmr)
...
def render_variant_config(cfg: dict[str, Any]) -> None:
    ...
    variant_cfg.append(shell_var("DIST_NAME", f"bgos-{name}"))
    variant_cfg.append(shell_var("DIST_VERSION", cfg["variant"].get("version", "0.0.0")))
    variant_cfg.append(shell_var("MODULES", " ".join(modules)))
    variant_cfg.append(shell_var("BGOS_VARIANT", name))
    variant_cfg.append(shell_var("BGOS_HOSTNAME", cfg["hostname"]))
    ...
    image_lines = [
        ...
        shell_var("BGOS_TARGETS", ",".join(cfg["targets"])),
    ]
